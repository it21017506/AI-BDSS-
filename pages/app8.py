...
st.set_page_config(
    ...
    page_icon='C:/Users/anura/Downloads/bar-chart_1553410.png',
    layout="wide",
...
def generate_detailed_suggestions(product, feedback_analysis, product_data):
    ...
    if avg_rating < 2.5:
        suggestions.append(" **Critical Issue**: Low customer satisfaction. Focus on quality improvement and customer service training.")
    elif 2.5<= avg_rating < 3.5:
        suggestions.append(" **Improvement Needed**: Moderate customer satisfaction. Implement feedback-based improvements.")
    else:
        suggestions.append(" **Good Performance**: High customer satisfaction. Maintain quality standards and consider expansion.")
    
    # Review volume suggestions
    if total_reviews < 5:
        suggestions.append(" **Limited Feedback**: Encourage more customer reviews to better understand product performance.")
    ...
    if sentiment.get('delivery_issues', 0) > 0:
        suggestions.append("**Delivery Issues**: Improve delivery speed and handling procedures. Consider logistics optimization.")
    ...
    if avg_sales < 100:
        ...
        suggestions.append("**High Performance**: Focus on maintaining market position and premium pricing strategies.")
            ...
            if X_qty is not None:
                ...
                if trained_models_qty:
                    ...
                    if future_predictions_qty:
                        ...
                        for step in range(6):
                            step_predictions = [pred[step] for pred in future_predictions_qty.values() if len(pred) > step]
                            if step_predictions:
            ...
            if X_sales is not None:
                ...
                if trained_models_sales:
                    ...
                    if future_predictions_sales:
                        ...
                        for step in range(6):
                            step_predictions = [pred[step] for pred in future_predictions_sales.values() if len(pred) > step]
                            if step_predictions:
    ...
    with tab2:
        ...
        if show_ensemble and scores:
            ...
            if target_variable == "Both":
                ...
                filtered_data = [row for row in performance_data if row['Target'].lower().replace(' ', '_') == target_selection]
                if filtered_data:
                ...
                target_type = 'quantity' if target_variable == "Quantity Only" else 'total_sales'
                filtered_data = [row for row in performance_data if row['Target'].lower().replace(' ', '_') == target_type]
                if filtered_data:
                    filtered_df = pd.DataFrame(filtered_data)
                    pivot_df = filtered_df.pivot(index='Product', columns='Model', values='R² Score')
                    pivot_df = pivot_df.apply(pd.to_numeric, errors='coerce')
...
def reinforcement_learning_page():
    ...
    st.markdown("This page uses reinforcement learning and customer feedback to provide detailed business recommendations.")
...
def display_enhanced_business_recommendations(recommendations, selected_products, feedback_data):
    ...
    tab1, tab2, tab3, tab4 = st.tabs([" Recommendations", " Performance Analysis", " Feedback Analysis", " Action Plan"])
            ...
            if product in recommendations:
                ...
                with st.container():
                    ...
                    if rec.get('feedback_analysis'):
                        ...
                        with col6:
                            positive_rate = (feedback.get('positive_reviews', 0) / feedback.get('total_reviews', 1)) * 100
